...
public class GlobalExceptionHandler {
    /**
     * Handles MissingServletRequestParameterException for missing parameters
     * in @RequestParam.
     ...
     * @return ResponseEntity with a general message and the required parameters for
    ...
    public ResponseEntity<Map<String, Object>> handleMissingServletRequestParameterException(
        ...
        String controllerName = handlerMethod != null ? handlerMethod.getBeanType().getSimpleName()
                : "UnknownController";

        // Get required parameters for the controller
        Map<String, String> requiredParameters = requiredParametersByController.getOrDefault(controllerName,
                Map.of());

        // Build response structure
        response.put("requiredParameters", requiredParameters);
        response.put("message",
                "Missing required parameters. Ensure the following parameters are included in the request:");
        return ResponseEntity.badRequest().body(response);
    }

    /**
     * Handles ConstraintViolationException for parameter validation errors.
     * Builds a response with specific parameter violations and a descriptive
     * message.
     *
    ...
    /**
     * Handles InvalidReturnDateException for custom validation of return and
     * departure dates.
     *
     * @param ex InvalidReturnDateException indicating return date issues
     * @return ResponseEntity with a structured error response for invalid return
     *         dates
     */
    ...
    /**
     * Handles MethodArgumentTypeMismatchException for invalid arguments in
     * parameters, such as enum mismatches.
    ...
    public ResponseEntity<Map<String, Object>> handleMethodArgumentTypeMismatch(
        ...
        String expectedTypeMessage = "Invalid value for parameter '%s'. %s".formatted(
                parameterName,
                ex.getRequiredType().equals(LocalDate.class) ? "Expected format: YYYY-MM-DD"
                        : "Allowed values: USD, MXN, EUR");
